...
| 5 | **Build Frontend** | Sul VPS: `cd frontend && npm run build` |
| 6 | **Build Backend** | Sul VPS: `cd backend && npm run build` |
| 7 | **Restart** | `pm2 restart gads-audit-backend --update-env` + copia frontend build |
...
# 5. Copia frontend build per Nginx
cp -r frontend/dist/* /var/www/gads-audit/public/
...
```bash
ssh root@185.192.97.108 'pm2 restart gads-audit-backend'
```

### Verifica Logs

```bash
ssh root@185.192.97.108 'pm2 logs gads-audit-backend --lines 20 --nostream'
```

### Verifica Status

```bash
ssh root@185.192.97.108 'pm2 show gads-audit-backend'
```
...
VPS_PATH="/var/www/gads-audit"      # Path sul server
BRANCH="main"                       # Branch Git
PM2_PROCESS="gads-audit-backend"    # Nome processo PM2
LOCAL_PORT_FRONTEND=5173             # Porta frontend in sviluppo locale
LOCAL_PORT_BACKEND=3002              # Porta backend in sviluppo locale
SERVER_PORT=3002                     # Porta backend sul server
PUBLIC_URL="https://gads.karalisweb.it"  # URL pubblico
NGINX_CONFIG="/etc/nginx/sites-available/gads.karalisweb.it"  # Config Nginx
```
...
Internet → Nginx (80/443)
                ├─ /api/* → proxy_pass http://localhost:3002 (NestJS backend)
                └─ /* → /var/www/gads-audit/public/ (React SPA static files)
...
PM2:
  └─ gads-audit-backend (NestJS, porta 3002)
```
...
4. **TypeORM Migrations**: Se ci sono modifiche allo schema del database:
   ```bash
   ssh root@185.192.97.108 'cd /var/www/gads-audit/backend && npm run migration:run'
   ```

5. **PM2 fallback**: Se il processo non esiste ancora, lo script lo crea automaticamente:
   ```bash
   pm2 start dist/main.js --name 'gads-audit-backend' -- start
   ```
...
### L'app non si avvia
```bash
ssh root@185.192.97.108 'pm2 logs gads-audit-backend --lines 50 --nostream'
```

### Errore durante la build frontend
```bash
ssh root@185.192.97.108 'cd /var/www/gads-audit/frontend && npm run build 2>&1 | tail -30'
```

### Errore durante la build backend
```bash
ssh root@185.192.97.108 'cd /var/www/gads-audit/backend && npm run build 2>&1 | tail -30'
```

### Verifica container Docker (DB + Redis)
```bash
ssh root@185.192.97.108 'cd /var/www/gads-audit && docker compose ps'
```

### Reinstalla dipendenze da zero
```bash
ssh root@185.192.97.108 'cd /var/www/gads-audit/frontend && rm -rf node_modules && npm install && npm run build'
ssh root@185.192.97.108 'cd /var/www/gads-audit/backend && rm -rf node_modules && npm install && npm run build && pm2 restart gads-audit-backend'
```
...
npm install
npm run build
pm2 start dist/main.js --name gads-audit-backend
pm2 save
```
...
```bash
# Creare config
nano /etc/nginx/sites-available/gads.karalisweb.it

# Abilitare
ln -s /etc/nginx/sites-available/gads.karalisweb.it /etc/nginx/sites-enabled/
nginx -t && systemctl reload nginx

# SSL
certbot --nginx -d gads.karalisweb.it
```
