 ...
 * ULTIMA MODIFICA: 2026-06-13 (v3.8: + stato salute conversioni - inattive)
 *
...
function exportDataset(accountId, runId, datasetName, totalDatasets) {
  ...
  switch (datasetName) {
    ...
    case 'conversion_action_metrics':
      data = extractConversionActionMetrics();
      break;
...
function addNegativeKeyword(mod) {
  var text = mod.afterValue.text || mod.afterValue.keyword;
  var matchType = mod.afterValue.matchType || 'BROAD';
  var level = (mod.afterValue.level || 'CAMPAIGN').toUpperCase();
  var parsed = parseEntityId(mod.entityId);
